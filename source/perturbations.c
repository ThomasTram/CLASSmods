...
int perturb_init(
  ...
  if (ppt->has_tensors == _TRUE_) {
    ...
    case (tm_massless_approximation):
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_inu == _TRUE_))
...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_delta_inu = _FALSE_;
  ...
  ppt->has_source_theta_inu = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_inu == _TRUE_)
          ppt->has_source_delta_inu = _TRUE_;
      }
	...
	if (pba->has_inu == _TRUE_)
          ppt->has_source_theta_inu = _TRUE_;
      }
      ...
      class_define_index(ppt->index_tp_delta_inu,  ppt->has_source_delta_inu, index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_inu,  ppt->has_source_theta_inu, index_type,1);
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (pba->has_inu == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_inu);
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
     ...
     /** NEW3: want z instead of a as an output **/
      ...
      class_store_columntitle(ppt->scalar_titles,"delta_inu",pba->has_inu);
      class_store_columntitle(ppt->scalar_titles,"theta_inu",pba->has_inu);
      class_store_columntitle(ppt->scalar_titles,"shear_inu",pba->has_inu);
      class_store_columntitle(ppt->scalar_titles,"cs2_inu",pba->has_inu);

      //NEW3:
      class_store_columntitle(ppt->scalar_titles,"inu_number_conservation",pba->has_inu);
      class_store_columntitle(ppt->scalar_titles,"inu_energy_conservation",pba->has_inu);
      class_store_columntitle(ppt->scalar_titles,"inu_momentum_conservation",pba->has_inu);
      class_store_columntitle(ppt->scalar_titles,"inu_quadrupole_violation",pba->has_inu);
    ...
    if (ppt->has_tensors == _TRUE_){
      ...
      class_store_columntitle(ppt->tensor_titles,"delta_inu",ppt->evolve_tensor_inu);
      class_store_columntitle(ppt->tensor_titles,"theta_inu",ppt->evolve_tensor_inu);
      class_store_columntitle(ppt->tensor_titles,"shear_inu",ppt->evolve_tensor_inu);

      // NEW3:
      class_store_columntitle(ppt->tensor_titles,"inu_number_conservation",ppt->evolve_tensor_inu);
      class_store_columntitle(ppt->tensor_titles,"inu_energy_conservation",ppt->evolve_tensor_inu);
      class_store_columntitle(ppt->tensor_titles,"inu_momentum_conservation",ppt->evolve_tensor_inu);
      class_store_columntitle(ppt->tensor_titles,"inu_quadrupole_violation",pba->has_inu);
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    /* interacting neutrinos */
    if (pba->has_inu == _TRUE_) { /* HERE??? */
      ppv->index_pt_psi0_inu = index_pt;
      // Set value of ppv->l_max_inu:
      /* reject inconsistent values of the number of mutipoles in ultra relativistic neutrino hierachy */
      class_test(ppr->l_max_inu < 4,
                 ppt->error_message,
                 "ppr->l_max_inu=%d should be at least 4, i.e. we must integrate at least over first four momenta of interacting dark matter perturbed phase-space distribution");
      //Copy value from precision parameter:
      ppv->l_max_inu = ppr->l_max_inu;
      ppv->q_size_inu = pba->q_size_inu;
      index_pt += (ppv->l_max_inu+1)*ppv->q_size_inu;
      class_define_index(ppv->index_pt_Delta_n_inu,_TRUE_,index_pt,1);
      class_define_index(ppv->index_pt_Delta_rho_inu,_TRUE_,index_pt,1);
      class_define_index(ppv->index_pt_Delta_momentum_inu,_TRUE_,index_pt,1);
    }
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_inu == _TRUE_) {
      ppv->index_pt_psi0_inu = index_pt;
      // Set value of ppv->l_max_inu:
      class_test(ppr->l_max_inu < 4,
                 ppt->error_message,
                 "ppr->l_max_inu should be at least 4, i.e. we must integrate at least over first four momenta of non-cold dark matter perturbed phase-space distribution",1);
      //Copy value from precision parameter:
      ppv->l_max_inu = ppr->l_max_inu;
      ppv->q_size_inu = pba->q_size_inu;
  ...
  if (_scalars_) {
    ...
    if (pba->has_inu == _TRUE_) {
      ...
      index_pt = ppv->index_pt_psi0_inu;
      for(index_q=0; index_q < ppv->q_size_inu; index_q++){
        for(l=0; l<=ppv->l_max_inu; l++){
          if (l>2) ppv->used_in_sources[index_pt]=_FALSE_;
          index_pt++;
        }
      }
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_inu == _TRUE_) { 
        ...
        for(index_q=0; index_q < ppv->q_size_inu; index_q++){
          for(l=0; l<=ppv->l_max_inu;l++){
            ppv->y[ppv->index_pt_psi0_inu+index_pt] =
              ppw->pv->y[ppw->pv->index_pt_psi0_inu+index_pt];
            index_pt++;
          }
        }
        ppv->y[ppv->index_pt_Delta_n_inu] = ppw->pv->y[ppw->pv->index_pt_Delta_n_inu];
        ppv->y[ppv->index_pt_Delta_rho_inu] = ppw->pv->y[ppw->pv->index_pt_Delta_rho_inu];
        ppv->y[ppv->index_pt_Delta_momentum_inu] = ppw->pv->y[ppw->pv->index_pt_Delta_momentum_inu];
      }
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    if (pba->has_inu == _TRUE_) {
      rho_r += ppw->pvecback[pba->index_bg_rho_inu];
      rho_nu += ppw->pvecback[pba->index_bg_rho_inu];
    }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* all relativistic relics: ur, early ncdm, dr, inu */
...
(pba->has_inu == _TRUE_)) {
    ...
    if ((ppt->has_nid == _TRUE_) && (index_ic == ppt->index_ic_nid)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_inu == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NID in absence of ur, ncdm or inu!");
    ...
    if ((ppt->has_niv == _TRUE_) && (index_ic == ppt->index_ic_niv)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_inu == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NIV in absence of ur, ncdm or inu!");
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_) || (pba->has_inu == _TRUE_)) {
   ...
   if (pba->has_inu == _TRUE_) {
     idx = ppw->pv->index_pt_psi0_inu;

     for (index_q=0; index_q < ppw->pv->q_size_inu; index_q++) {

       ppw->pv->y[idx] = -0.25 * delta_ur * pba->dlnf0_dlnq_inu[index_q];

       ppw->pv->y[idx+1] = -1.0/3./k*theta_ur* pba->dlnf0_dlnq_inu[index_q];

       /* Debugging IC */

       ppw->pv->y[idx+2] = -0.5 * shear_ur * pba->dlnf0_dlnq_inu[index_q];

       ppw->pv->y[idx+3] = -0.25 * l3_ur * pba->dlnf0_dlnq_inu[index_q];
...
int perturb_approximations(
  ...
  if (_scalars_) {
    ...
    /* Isabel: Do I need to include sth for inu here? */
    /* Thomas: Not yet, so far we do not have any fluid approximation for inu */
...
int perturb_total_stress_energy(
  ...
  double n_delta_inu=0.;
  double rho_delta_inu=0.;
  double rho_plus_p_theta_inu=0.;
  double rho_plus_p_shear_inu=0.;
  double delta_p_inu=0.;
  double factor;
  double rho_plus_p_ncdm;
  int index_q,n_ncdm,idx;
  double epsilon,q,q2,cg2_ncdm,w_ncdm,rho_ncdm_bg,p_ncdm_bg,pseudo_p_ncdm;
  double rho_plus_p_inu;
  double w_inu,rho_inu_bg,p_inu_bg;
  ...
  if (_scalars_) {
    ...
    /* interacting neutrino contribution */
    if (pba->has_inu == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_inu;
      // We must integrate to find perturbations:
      n_delta_inu = 0.0;
      rho_delta_inu = 0.0;
      rho_plus_p_theta_inu = 0.0;
      rho_plus_p_shear_inu = 0.0;
      delta_p_inu = 0.0;
      factor = pba->factor_inu*pow(pba->a_today/a,4);
	...
	n_delta_inu += q2*pba->w_inu[index_q]*y[idx];
        rho_delta_inu += q2*q*pba->w_inu[index_q]*y[idx];
        rho_plus_p_theta_inu += q2*q*pba->w_inu[index_q]*y[idx+1];
        rho_plus_p_shear_inu += q2*q*pba->w_inu[index_q]*y[idx+2];
        delta_p_inu += q2*q*pba->w_inu[index_q]*y[idx];       
    ...
    /*  inu_number_conservation = pba->factor_inu*pow(pba->a_today/a,3)/(pba->T_cmb*pba->T_inu*_k_B_)*y[ppw->pv->index_pt_Delta_n_inu]/n_delta_inu;
      ...
      inu_momentum_conservation = pba->factor_inu*pow(pba->a_today/a,4)*k*y[ppw->pv->index_pt_Delta_momentum_inu]/rho_plus_p_theta_inu;
      inu_quadrupole_violation = 0.; */

      /* Isabel: Multiplying by number of inu, since pvecback[pba->index_bg_rho_inu] is defined for ALL species, but rho_delta_inu only for ONE species.*/
      /* Thomas: We will compute the total energy density perturbation for inu, then we don't need N_inu. (We do this by changing pba->factor_inu). */
      if ((ppt->has_source_delta_inu == _TRUE_) || (ppt->has_source_theta_inu == _TRUE_)) {
        ppw->delta_inu = rho_delta_inu/ppw->pvecback[pba->index_bg_rho_inu];
        ppw->theta_inu = rho_plus_p_theta_inu/
      ...
      /* Isabel: Multiplying by number of inu to obtain the right contribution to rho etc. */
      /* Thomas: We can again avoid this. pba->N_inu is effectively stored in pba->inu_factor. */
      ppw->delta_rho += rho_delta_inu;
      ppw->rho_plus_p_theta += rho_plus_p_theta_inu;
      ppw->rho_plus_p_shear += rho_plus_p_shear_inu;
      ppw->delta_p += delta_p_inu;
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_){
      ...
      if (ppt->tensor_method == tm_massless_approximation) {
        ...
        if (pba->has_inu == _TRUE_) {
            rho_relativistic += ppw->pvecback[pba->index_bg_rho_inu];
        }
     }
    ...
    /* Isabel: No need to include such a routine for inu, since inu is already accounted for in rho_relativistic? */
    /* Thomas: In principle we should include it, since the shear properties that source gravitational waves
       can be very different from massless species. In the massless boson case there may not be any shear. However,
       let us leave it for later.
    */
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_inu */
    if (ppt->has_source_delta_inu == _TRUE_) {
        _set_source_(ppt->index_tp_delta_inu) = ppw->delta_inu;
    }
    ...
    /* theta_inu */
    if (ppt->has_source_theta_inu == _TRUE_) {
        _set_source_(ppt->index_tp_theta_inu) =  ppw->theta_inu;
    }
  }
...
int perturb_print_variables(double tau,
  ...
  /** ncdm sector ends */
 /** inu sector begin */
  double delta_inu=0., theta_inu=0., shear_inu=0., delta_p_over_delta_rho_inu=0.;
  double inu_number_conservation=0., inu_energy_conservation=0., inu_momentum_conservation=0., inu_quadrupole_violation=0.;
  double rho_inu_bg, p_inu_bg, w_inu;
  double rho_delta_inu = 0.0;
  double n_delta_inu= 0.0;
  double rho_plus_p_theta_inu = 0.0;
  double rho_plus_p_shear_inu = 0.0;
  double delta_p_inu = 0.0;
  /** inu sector ends */
  ...
  if (_scalars_) {
    ...
    if (0==1){//(ppt->gauge == synchronous) {
      ...
      /* Isabel: Do I have to include here sth? Yes*/
      if (pba->has_inu == _TRUE_) {
        delta_inu -= 4. * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
        theta_inu += k*k*alpha;
      }
...
/*NEW3: want z instead of a as output */
    ...
    /* Interacting neutrinos */
    class_store_double(dataptr, delta_inu, pba->has_inu, storeidx);
    class_store_double(dataptr, theta_inu, pba->has_inu, storeidx);
    class_store_double(dataptr, shear_inu, pba->has_inu, storeidx);
    class_store_double(dataptr, delta_p_over_delta_rho_inu,  pba->has_inu, storeidx);
    //NEW3:
    class_store_double(dataptr, inu_number_conservation, pba->has_inu, storeidx);
    class_store_double(dataptr, inu_energy_conservation, pba->has_inu, storeidx);
    class_store_double(dataptr, inu_momentum_conservation, pba->has_inu, storeidx);
    class_store_double(dataptr, inu_quadrupole_violation, pba->has_inu, storeidx);
    /* Decaying cold dark matter */
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_inu == _TRUE_) {
      ...
      class_store_double(dataptr, delta_inu, _TRUE_, storeidx);
      class_store_double(dataptr, theta_inu, _TRUE_, storeidx);
      class_store_double(dataptr, shear_inu, _TRUE_, storeidx);

      //NEW3: 
      class_store_double(dataptr, inu_number_conservation, _TRUE_, storeidx);
      class_store_double(dataptr, inu_energy_conservation, _TRUE_, storeidx);
      class_store_double(dataptr, inu_momentum_conservation, _TRUE_, storeidx);
      class_store_double(dataptr, inu_quadrupole_violation, _TRUE_, storeidx);
...
int perturb_derivs(double tau,
  ...
  double * S; 
  double Norm;
  double rho_inu_bg,p_inu_bg, G_eff, T_inu, w_inu;
  double f0;
  int index_qpr, index_l, lmax, Nq;
  double S_0, S_1;
 ...
 /* For G_eff_CYR =0 we recover the non-interacting ur neutrino case... Add an if-condition to switch off interactions maybe later?  **/

          double taudot2, taudot3, taudot4, taudot5, taudot;
          taudot2 = -0.40/pow(a,4)*pow(pba->T_inu*pba->T_cmb*_k_B_,5)*pow(pba->G_eff_CYR/(1e12*_eV_*_eV_),2)*(2.*_PI_/_h_P_)/_c_*_Mpc_over_m_;
          taudot3 = -0.43/pow(a,4)*pow(pba->T_inu*pba->T_cmb*_k_B_,5)*pow(pba->G_eff_CYR/(1e12*_eV_*_eV_),2)*(2.*_PI_/_h_P_)/_c_*_Mpc_over_m_;
          taudot4 = -0.46/pow(a,4)*pow(pba->T_inu*pba->T_cmb*_k_B_,5)*pow(pba->G_eff_CYR/(1e12*_eV_*_eV_),2)*(2.*_PI_/_h_P_)/_c_*_Mpc_over_m_;
	  ...
	  dy[pv->index_pt_delta_ur+4] +=  taudot4*y[pv->index_pt_delta_ur+l];
...
/** Isabel: This hierarchy is eq. (6.6) in astro-ph/1409.1577 devided by f0(q). */
    ...
    if (pba->has_inu == _TRUE_) {
      ...
      double C3term, qpr, f0qpr;

      idx = pv->index_pt_psi0_inu;

      /** ---> use exact equation (Boltzmann hierarchy on momentum grid) */
      ...
      for (index_q=0; index_q < pv->q_size_inu; index_q++) {
       ...
       /* dy_scat = ppw->dy_scat; */
        ...
        printf("===== index_q = %d =====\n",index_q);
        for(index_l=0; index_l <= lmax; index_l++){
          printf("l=%d, Psi = %.2e, dPsi/dtau_1 = %.2e, dPsi/dtau_2 = %.2e\n",index_l,y[idx+index_l],dy[idx+index_l],Norm*ppw->dy_scat[index_l]/f0);
        }
        */
      ...
      dy[pv->index_pt_Delta_n_inu] = 0.;
      dy[pv->index_pt_Delta_rho_inu] = 0.;
      dy[pv->index_pt_Delta_momentum_inu] = 0.;
	...
	for (index_q=0; index_q<pv->q_size_inu; index_q++){
     ...
     /* dy[pv->index_pt_Delta_n_inu] *= Norm*pba->factor_inu*pow(pba->a_today/a,3)/(pba->T_cmb*pba->T_inu*_k_B_);
      dy[pv->index_pt_Delta_rho_inu] *= Norm*pba->factor_inu*pow(pba->a_today/a,4);
      dy[pv->index_pt_Delta_momentum_inu] *= Norm*pba->factor_inu*pow(pba->a_today/a,4)*k; */

      dy[pv->index_pt_Delta_n_inu] *= Norm;
      dy[pv->index_pt_Delta_rho_inu] *= Norm;
      dy[pv->index_pt_Delta_momentum_inu] *= Norm; 
  ...
  if (_tensors_) {
    ...
    /** -> interacting neutrinos (inu) */
    /* Isabel: Still have to modify hierarchy here! */
    //TBC: curvature in all inu
    if (ppt->evolve_tensor_inu == _TRUE_) {
      ...
      idx = pv->index_pt_psi0_inu;
        ...
        for (index_q=0; index_q < pv->q_size_inu; index_q++) {
          ...
          dy[idx] = -k*y[idx+1]-0.25*_SQRT6_*y[pv->index_pt_gwdot]*dlnf0_dlnq;

          /** -----> inu l>0 for given momentum bin */

          for(l=1; l<pv->l_max_inu; l++){
            dy[idx+l] = k/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
          }
...
int compute_full_scatter(double *S2, struct background * pba, struct perturbs * ppt, int lmax){
  ...
  /* Don't I have to free all these pointers here? If I free them, I get an error message...
     They have to be free, but only AFTER the index_l loop where they are used! */
  /* free(data); data should never have been allocated in the first place :) */
