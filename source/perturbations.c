...
int perturb_workspace_init(
  ...
  ppw->HCtheta_p_prime = 0.0;
  ppw->HCA_nb_prime = 0.0;
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      class_store_columntitle(ppt->scalar_titles, "HCtheta", ppt->has_spatial_gauge_transfers);
      class_store_columntitle(ppt->scalar_titles, "HCtheta_prime", ppt->has_spatial_gauge_transfers);
      ...
      class_store_columntitle(ppt->scalar_titles, "CHT_grow", ppt->has_spatial_gauge_transfers);
      class_store_columntitle(ppt->scalar_titles, "CHT_decay", ppt->has_spatial_gauge_transfers);
...
int perturb_vector_init(
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      /**
	  ppw->HCA_nb_old =  a_prime_over_a /rho_plus_p_tot * 
	  ...
	  ppw->HCtheta_p_old =  a_prime_over_a /rho_plus_p_tot * ppw->rho_plus_p_theta;
      */

      /* -- case of switching on L evolution. Provide correct initial conditions to new set of variables */
      
      if ((pa_old[ppw->index_ap_levo] == (int)levo_off) && (ppw->approx[ppw->index_ap_levo] == (int)levo_on)) {
        double rho_plus_p_tot = 2./3.*(pba->K/a/a-ppw->pvecback[pba->index_bg_H_prime]/a);
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_L_prime == _TRUE_) {
      ...
      double L_gauge = 0., L_prime_gauge = 0.;
      if (ppw->approx[ppw->index_ap_levo] == (int)levo_on)
        L_gauge = y[ppw->pv->index_pt_L];
			
      if (ppw->approx[ppw->index_ap_levo] == (int)levo_on)
        L_prime_gauge = y[ppw->pv->index_pt_L_prime];
      ...
      _set_source_(  ppt->index_tp_CHT_grow) = CHT_grow;
      _set_source_(  ppt->index_tp_CHT_decay) = CHT_decay;
    }
...
int perturb_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    double delta_cdm_plus_b_nb = (ppw->pvecback[pba->index_bg_rho_cdm]
    ...
    class_store_double(dataptr, ppw->HCA_nb_old / ppw->pvecback[pba->index_bg_H] / ppw->pvecback[pba->index_bg_a], ppt->has_spatial_gauge_transfers,storeidx);
    ...
    class_store_double(dataptr, ppw->HCtheta_p_old, ppt->has_spatial_gauge_transfers,storeidx);
    class_store_double(dataptr, ppw->HCtheta_p_prime, ppt->has_spatial_gauge_transfers,storeidx);
    ...
    if (ppw->approx[ppw->index_ap_levo] == (int)levo_on){
      class_store_double(dataptr, y[ppw->pv->index_pt_CHT_grow], ppt->has_spatial_gauge_transfers,storeidx);
      class_store_double(dataptr, y[ppw->pv->index_pt_CHT_decay], ppt->has_spatial_gauge_transfers,storeidx);
    }
    else{
...
int perturb_store_timestep(double tau,
                            double * y,
                            double * dy,
                            void * parameters_and_workspace,
                            ErrorMsg error_message
  ...
  if (_scalars_) {
    ...
    double HCA_nb =  a_prime_over_a /rho_plus_p_tot * 
          (ppw->rho_plus_p_shear - ppw->delta_p - a_prime_over_a/k/k *
           (4./3. *ppw->pvecback[pba->index_bg_rho_g] + 4./3. *ppw->pvecback[pba->index_bg_rho_ur])/rho_plus_p_tot *ppw->rho_plus_p_theta );
    double HCtheta_p = a_prime_over_a /rho_plus_p_tot * ppw->rho_plus_p_theta;
  
    if (fabs(tau-ppw->tau_old)>1e-6) {

      //ppw->HCA_nb_prime =(HCA_nb - ppw->HCA_nb_old)/(tau - ppw->tau_old);
      //ppw->HCtheta_p_prime = (HCtheta_p - ppw->HCtheta_p_old)/(tau - ppw->tau_old);
      //ppw->HCtheta_p_prime = a_prime_over_a*k*k*(pvecmetric[ppw->index_mt_psi]- HCA_nb/a_prime_over_a)-1.5*a2*ppw->rho_plus_p_theta;
      /**
      ppw->HCA_nb_prime = -ppw->HCA_nb_prime + 2*(HCA_nb - ppw->HCA_nb_old)/(tau - ppw->tau_old);
      ppw->HCtheta_p_prime = -ppw->HCtheta_p_prime + 2*(HCtheta_p - ppw->HCtheta_p_old)/(tau - ppw->tau_old);
      ppw->HCtheta_p_old = HCtheta_p;
      ppw->HCA_nb_old = HCA_nb;
      ppw->tau_old = tau;*/
      //printf("%.16e %.16e %.16e %.16e %.16e %.16e\n", tau, k ,ppw->HCA_nb_old, ppw->HCA_nb_prime, ppw->HCtheta_p_old, ppw->HCtheta_p_prime);
    }
  }

  if (ppw->index_ikout!=-1){
    class_call(perturb_print_variables(tau, y,dy, parameters_and_workspace,error_message),
...
int perturb_derivs(double tau,
  ...
  /* for use with L displacement */
  double shear_ur_prime=0., shear_g_prime=0., delta_g_prime=0., delta_ur_prime=0., delta_ur=0.0, shear_ur=0.0;
  double p_ur=0.0, p_g=0.0;
  ...
  if (_scalars_) {
    ...
    if (ppt->has_spatial_gauge_transfers == _TRUE_) {
      ...
      /**
         ppw->HCA_nb =  a_prime_over_a /rho_plus_p_tot *
      ...
      */
      ...
      ppw->HCtheta_p_old = ppw->HCtheta_p;

      //ppw->HCtheta_p_prime = a_prime_over_a*k*k*(pvecmetric[ppw->index_mt_psi]- ppw->HCA_nb/a_prime_over_a)-1.5*a2*ppw->rho_plus_p_theta;
      //ppw->HCtheta_p_prime = -k*k*(ppw->HCA_nb+pvecmetric[ppw->index_mt_phi_prime]);
      /**
         ppw->HCtheta_p_prime = -k*k*pvecmetric[ppw->index_mt_phi_prime]+
        4*p_tot_real/rho_plus_p_tot*ppw->HCtheta_p+
        a_prime_over_a*k*k/rho_plus_p_tot*(ppw->delta_p-ppw->rho_plus_p_shear);*/
      ppw->HCtheta_p_prime = pvecback[pba->index_bg_H_prime]/pvecback[pba->index_bg_H]*ppw->HCtheta_p-
        p_tot_prime/rho_plus_p_tot*ppw->HCtheta_p+a_prime_over_a*k*k/rho_plus_p_tot*
        (ppw->delta_p-ppw->rho_plus_p_shear+rho_plus_p_tot*pvecmetric[ppw->index_mt_psi]);
      //if (fabs(1.0-fabs(ppw->HCA_nb/pvecmetric[ppw->index_mt_phi_prime]))<1e-3)
      //if (tau<500)
      //  printf("tau = %g, %.16e %.16e %.16e\n",tau,ppw->HCtheta_p_prime,-k*k*ppw->HCA_nb,-k*k*pvecmetric[ppw->index_mt_phi_prime]);

      // HCtheta_p is the multiplication of Hc and theta in Poisson gauge
      if (pba->has_ur==_TRUE_){
        if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off){
          delta_ur_prime = dy[ppw->pv->index_pt_delta_ur];
          ...
          delta_ur = ppw->rsa_delta_ur;
          delta_ur_prime = -4.*pvecmetric[ppw->index_mt_phi_prime];
        }
        p_ur = 1./3.*pvecback[pba->index_bg_rho_ur];
      }
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off){
        delta_g_prime = dy[pv->index_pt_delta_g];
        if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
          shear_g_prime = dy[pv->index_pt_shear_g];
        }
      }
      else{
