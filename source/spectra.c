  ...
  if ((ppt->has_pk == _TRUE_) || (ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_)) {
    ...
    if (ppt->has_pk == _TRUE_) {
      ...
      for (index_pk=0; index_pk<psp->pk_size; index_pk++){
        class_call(spectra_pk(pba,ppt,ppm,pnl,psp,index_pk),
                   psp->error_message,
                   psp->error_message);
      }
    }
...
int spectra_indices(
  ...
  if (ppt->has_pk == _TRUE_) {
    ...
    if (ppt->has_source_disp_b == _TRUE_){
      psp->index_pk_disp_b = index_pk;
      psp->index_tp_for_pk[index_pk] = ppt->index_tp_disp_b;
      index_pk++;
    }
    if (ppt->has_source_disp_cdm == _TRUE_){
      psp->index_pk_disp_cdm = index_pk;
      psp->index_tp_for_pk[index_pk] = ppt->index_tp_disp_cdm;
      index_pk++;
    }
  ...
  /* indices for auxillary transfer functions */
  class_define_index(psp->index_tr_phi,ppt->has_source_phi,index_tr,1);
  class_define_index(psp->index_tr_xi,ppt->has_source_xi,index_tr,1);
  class_define_index(psp->index_tr_xi_prime,ppt->has_source_xi,index_tr,1);
...
int spectra_pk(
  ...
  class_alloc(psp->ln_pk[index_pk],
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++) {
        ...
        source_ic1 = ppt->sources[index_md]
          [index_ic1 * ppt->tp_size[index_md] + psp->index_tp_for_pk[index_pk]]
        ...
        for (index_ic2 = index_ic1+1; index_ic2 < psp->ic_size[index_md]; index_ic2++) {
          ...
          if (psp->is_non_zero[index_md][index_ic1_ic2] == _TRUE_) {
            ...
            source_ic2 = ppt->sources[index_md]
              [index_ic2 * ppt->tp_size[index_md] + psp->index_tp_for_pk[index_pk]]
              [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];
      ...
      if ((pnl->method != nl_none)&&(index_pk == psp->index_pk_delta_m)) {
...
int spectra_sigma(
  ...
  for (i=0;i<psp->ln_k_size;i++) {
    ...
    class_call(spectra_pk_at_k_and_z(pba,ppm,psp,k,z,&pk,pk_ic,psp->index_pk_delta_m),
               psp->error_message,
               psp->error_message);
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
        ...
        if (ppt->has_source_xi == _TRUE_) {
           ...
           psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_xi] =
             ppt->sources[index_md]
             [index_ic * ppt->tp_size[index_md] + ppt->index_tp_xi]
             [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

           psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_xi_prime] =
             ppt->sources[index_md]
             [index_ic * ppt->tp_size[index_md] + ppt->index_tp_xi_prime]
             [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];
...
int spectra_output_tk_titles(struct background *pba,
  ...
  if (output_format == class_format) {
    ...
    class_store_columntitle(titles,"xi",ppt->has_source_xi);
    class_store_columntitle(titles,"xi_prime",ppt->has_source_xi);
  }
    ...
    for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
      ...
      for (index_k=0; index_k<psp->ln_k_size; index_k++) {
        ...
        if (output_format == class_format) {
          ...
          class_store_double(dataptr,tk[psp->index_tr_xi],ppt->has_source_xi,storeidx);
          class_store_double(dataptr,tk[psp->index_tr_xi_prime],ppt->has_source_xi,storeidx);
